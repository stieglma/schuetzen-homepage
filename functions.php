<?php
/**
 * Edelweiss Gaishofen functions and definitions
 *
 ...
 */
function edelweiss_setup() {
    // Make theme available for translation
    load_theme_textdomain('edelweiss-gaishofen', get_template_directory() . '/languages');
    ...
    add_image_size('edelweiss-same-size-thumb', 350, 300, true);
    add_image_size('edelweiss-gallery-thumb', 200, 133, true);
}
add_action('after_setup_theme', 'edelweiss_setup');

/**
 * Set the content width in pixels, based on the theme's design and stylesheet.
 */
function edelweiss_content_width() {
    $GLOBALS['content_width'] = apply_filters('edelweiss_content_width', 1200);
}
add_action('after_setup_theme', 'edelweiss_content_width', 0);

/**
 * Enqueue scripts and styles.
 */
function edelweiss_scripts() {
    // Enqueue Google Fonts
    wp_enqueue_style(
        'edelweiss-google-fonts',
    ...
    wp_localize_script('edelweiss-script', 'edelweiss_ajax_object', array(
        'ajax_url' => admin_url('admin-ajax.php'),
        'nonce'    => wp_create_nonce('edelweiss_nonce'),
        'strings'  => array(
            'read_more' => __('Read More', 'edelweiss-gaishofen'),
            'collapse'  => __('Collapse', 'edelweiss-gaishofen'),
        ),
    ));
}
add_action('wp_enqueue_scripts', 'edelweiss_scripts');

/**
 * Custom excerpt length.
 */
function edelweiss_excerpt_length($length) {
    return 30;
}
add_filter('excerpt_length', 'edelweiss_excerpt_length', 999);

/**
 * Custom excerpt more string.
 */
function edelweiss_excerpt_more($more) {
    return '...';
}
add_filter('excerpt_more', 'edelweiss_excerpt_more');

/**
 * Fallback menu for when no menu is assigned
 */
function edelweiss_fallback_menu() {
    $menu_items = array(
        '#home' => __('Start', 'edelweiss-gaishofen'),
        '#about' => __('About Us', 'edelweiss-gaishofen'),
 ...
 */
function edelweiss_add_meta_boxes() {
    add_meta_box(
        'edelweiss_post_options',
        __('Post Options', 'edelweiss-gaishofen'),
        'edelweiss_post_options_callback',
        'post',
        'normal',
        'default'
    );
}
add_action('add_meta_boxes', 'edelweiss_add_meta_boxes');

/**
 * Meta box callback function
 */
function edelweiss_post_options_callback($post) {
    wp_nonce_field('edelweiss_save_meta_box_data', 'edelweiss_meta_box_nonce');

    $short_post = get_post_meta($post->ID, '_edelweiss_short_post', true);

    echo '<table class="form-table">';
    echo '<tr>';
    echo '<th scope="row"><label for="edelweiss_short_post">' . __('Short Post', 'edelweiss-gaishofen') . '</label></th>';
    echo '<td><input type="checkbox" id="edelweiss_short_post" name="edelweiss_short_post" value="1" ' . checked(1, $short_post, false) . ' />';
    echo '<p class="description">' . __('Check this if this is a short post (no "Read More" button will be shown)', 'edelweiss-gaishofen') . '</p></td>';
    echo '</tr>';
    echo '</table>';
}
 ...
 * Save meta box data
 */
function edelweiss_save_meta_box_data($post_id) {
    if (!isset($_POST['edelweiss_meta_box_nonce'])) {
        return;
    }

    if (!wp_verify_nonce($_POST['edelweiss_meta_box_nonce'], 'edelweiss_save_meta_box_data')) {
        return;
    }
    ...
    if (isset($_POST['edelweiss_short_post'])) {
        update_post_meta($post_id, '_edelweiss_short_post', 1);
    } else {
        delete_post_meta($post_id, '_edelweiss_short_post');
    }

}
add_action('save_post', 'edelweiss_save_meta_box_data');
...
/**
 * Include team members functionality
 */
 ...
 */
function edelweiss_training_page_admin_notice() {
    ...
    if ($content_source === 'page' && !$training_page_id) {
        echo '<div class="notice notice-info"><p>';
        printf(
            __('The training section is set to use a WordPress page, but no page is selected. <a href="%s">Configure training section</a> or <a href="%s">view the template</a> for creating a training page.', 'edelweiss-gaishofen'),
            admin_url('customize.php?autofocus[section]=edelweiss_training_section'),
            get_template_directory_uri() . '/training-page-template.html'
        );
        echo '</p></div>';
    }
}
add_action('admin_notices', 'edelweiss_training_page_admin_notice');

/**
 * Add training template to page creation
 */
function edelweiss_add_training_page_template_option() {
    global $post;
    if ($post && $post->post_type === 'page') {
        $template_file = get_template_directory() . '/training-page-template.html';
        if (file_exists($template_file)) {
            echo '<div id="training-template-notice" class="notice notice-info" style="margin: 20px 0;">';
            echo '<p><strong>' . __('Training Page Template Available', 'edelweiss-gaishofen') . '</strong></p>';
            echo '<p>' . __('Looking to create a training page? We have a pre-built template with professional styling and structure.', 'edelweiss-gaishofen') . '</p>';
            echo '<p>';
            echo '<a href="' . get_template_directory_uri() . '/training-page-template.html" target="_blank" class="button">';
            echo __('View Training Template', 'edelweiss-gaishofen');
            echo '</a> ';
            echo '<button type="button" class="button-link" onclick="document.getElementById(\'training-template-notice\').style.display=\'none\'">';
            echo __('Dismiss', 'edelweiss-gaishofen');
            echo '</button>';
            echo '</p>';
            echo '</div>';
        }
...
add_action('edit_form_after_title', 'edelweiss_add_training_page_template_option');